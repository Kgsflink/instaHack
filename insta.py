...
def getMediaLinks(userId, sessionId):
    """
    Fetch all image and video links from the user's profile.
    """
    media_links = []
    next_max_id = None

    while True:
        url = f'https://i.instagram.com/api/v1/feed/user/{userId}/?max_id={next_max_id}' if next_max_id else f'https://i.instagram.com/api/v1/feed/user/{userId}/'
        response = requests.get(
            url,
            headers={'User-Agent': 'Instagram 64.0.0.14.96'},
            cookies={'sessionid': sessionId}
        ...
        for item in data.get("items", []):
            if item.get("media_type") == 1:  # Image
                media_links.append(item["image_versions2"]["candidates"][0]["url"])
            elif item.get("media_type") == 2:  # Video
                media_links.append(item["video_versions"][0]["url"])
...
def main():
    ...
    # Fetch media links
    media_links = getMediaLinks(infos["userID"], sessionsId)
    print("\nMedia Links:")
    ...
    if other_infos["error"] == "rate limit":
        ...
        if "obfuscated_email" in other_infos["user"].keys():
            if other_infos["user"]["obfuscated_email"]:
                print("Raw email              : " + other_infos["user"]["obfuscated_email"])
            else:
                print("No email found")

        if "obfuscated_phone" in other_infos["user"].keys():
            if str(other_infos["user"]["obfuscated_phone"]):
                print("Raw phone              : " + str(other_infos["user"]["obfuscated_phone"]))
            else:
